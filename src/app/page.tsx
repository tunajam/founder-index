...
import { uniqueStocks } from "@/data/stocks"
import { getBacktestData, getBacktestSummary } from "@/data/backtest"
...
export default function Home() {
  ...
  }, [networkMin, founderMin])

  const backtestData = useMemo(() => getBacktestData(period), [period])
  const summary = useMemo(() => getBacktestSummary(period), [period])
    ...
    <div className="min-h-screen bg-stone-950 text-stone-100">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <Badge variant="outline" className="border-amber-500/30 text-amber-400 text-[10px]">
              {uniqueStocks.length} stocks scored
            </Badge>
            {!isPaid && (
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-8">
        ...
        <footer className="text-center py-8 text-xs text-stone-600">
          ...
          <p className="mt-1">Built by <a href="https://tunajam.com" className="text-amber-600 hover:text-amber-500">Tunajam</a></p>
        </footer>
      </main>
    </div>
  )
